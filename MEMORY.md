...
- ✅ **[RESOLVIDO 2026-05-23] Incidente de segurança — conta Cloudflare comprometida.** Atacante (IP `45.66.208.28`) injetou malware fake-update via **Zaraz** nas 6 zonas (22/05 16:40 UTC). **P0 lockout + P1 limpeza concluídos e verificados via API**: senha+2FA (Gmail+CF), todos os tokens rotacionados + god-token deletado + Global Key + OAuth apps revogados; Zaraz `tools=0` nas 6 zonas. **Worker/e-mail/segredos íntegros (não expostos).** C2 via EtherHiding (contrato Polygon) → `travel-js-ns.beer`. Relatório completo em [`INCIDENT-cloudflare-account-compromise-2026-05-22.md`](INCIDENT-cloudflare-account-compromise-2026-05-22.md) **§7**. **P2 pendente (não-urgente):** comunicar clientes do `membros.*`, travar `.com.br` no registro.br, ligar Cloudflare Notifications, denunciar C2, deletar `CF_AUDIT_TOKEN` (expira 31/05). Memória: `incident_cloudflare_compromise_2026_05_22.md`.
...
- ✅ **[2026-05-23] Recovery via Unnichat — IMPLEMENTADO, CORRIGIDO e LIVE.** (Já estava deployado desde 20/05 mas 85% falhava.) **2 bugs corrigidos hoje:** (1) `Contact not found` → `ensureUnnichatContact` (search→create contato antes de enviar); (2) link quebrado → URL param vira `Fn4XA0` com `?off=` na parte estática do template (WhatsApp percent-encoda `?`/`=` no `{{1}}` dinâmico). Deploy edge `65d5674e`, migrations 0056+0057, commits `6d008b4`+`e8581b8`. Validado (canário + 2 testes reais). Campanha `wcs_jun26_bait_abandono` **ativa**. **Pendência do user:** re-enfileirar os 62 jobs antigos que saíram com link quebrado. **[SYNC-PENDING]:** código cita "BR-RECOVERY" que ainda não existe em `docs/50-business-rules/`. Refs: `reference_unnichat_api.md`, [`docs/40-integrations/15-unnichat-whatsapp.md`](docs/40-integrations/15-unnichat-whatsapp.md), ADR-048.
- **Sprint ativo**: tags + recovery Unnichat em prod e funcionando; incidente CF resolvido. Sessão encerrada para zerar contexto.
- **Branch**: `main`. Pushed até `e8581b8` (recovery fix). Untracked locais (CSVs com PII, `INCIDENT-*.md`, COMMERCE_PLAN, facebook_docs, tmp/) **não** vão pro git — `leads_*.csv` + `tmp/` agora no `.gitignore`.
- **Edge prod**: último deploy **`65d5674e`** (recovery contact-flow fix, 2026-05-23) cobre o HEAD. Sem deploy pendente. Token de deploy mínimo novo em `.env.local` (god-token antigo revogado no incidente).
...
- **LEAD-ERASE-WORKER-EVENTS-SCOPE** (Sprint 18, 2026-05-18) — Novo worker `apps/edge/src/lib/lead-erase.ts` (consumido via `QUEUE_DISPATCH` por `POST /v1/leads/bulk-delete`) hoje NULL apenas PII de `leads` + DELETE `lead_aliases` + audit `erase_sar_completed`. NÃO toca `events.user_data` / `events.visitor_id` / `lead_attribution` (responsabilidade que segue no worker legado `apps/edge/src/lib/erasure.ts`). Convergir os dois workers (ou ter `lead-erase.ts` chamar a rotina de anonimização completa de events do worker legado) é dívida; sem isso, bulk-delete não cumpre integralmente BR-PRIVACY-005. Doc atualizada em `BR-PRIVACY.md` (BR-PRIVACY-005 / Enforcement). ETA: próxima sprint que tocar erasure (alinhar com ERASURE-GEO-FIELDS-AND-VISITOR-ID abaixo).
