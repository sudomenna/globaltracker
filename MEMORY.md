...
- **CP-DOUBLE-STRINGIFY-workspaces.config (NOVO)**: o mesmo bug de double-stringify do `pages.event_config` (T-13-013) também afeta `workspaces.config`. Detectado durante config do SendFlow campaign_map em 2026-05-05. Fix runtime aplicado: `UPDATE workspaces SET config = (config #>> '{}')::jsonb WHERE jsonb_typeof(config)='string'`. Apurar com T-13-013 — provavelmente mesmo handler do CP. **Pendência adicional**: rotinas que escrevem em `workspaces.config` precisam revisar a serialização.
