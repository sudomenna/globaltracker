...
- **GA4-NO-CLIENT-ID-LOOKUP-OQ-012 — RESOLVIDO 2026-05-07** via ADR-032 (cascata 4 níveis Alternativa D, não C como originalmente proposto). Auditoria do dia revelou que sibling lookup já existia mas todos os 15 Purchase Guru históricos tinham 0 events prévios com `_ga` (caso edge: comprador direto no checkout sem passar pela LP) — Alt C sozinha não cobre. Solução final: cascata 4 níveis em `apps/edge/src/dispatchers/ga4-mp/client-id-resolver.ts` (`resolveClientIdExtended`):
  1. **self** — resolveClientId(event.user_data) [já existia]
  2. **sibling** — _ga/fvid de evento anterior do mesmo lead, com filtro temporal `received_at < current` [refinado: filtro temporal era ausente]
  3. **cross_lead** — _ga/fvid de evento de OUTRO lead com mesmo phone_hash_external (1ª) ou email_hash_external (2ª) [novo]
  ...
  - **Backfill EXECUTADO 2026-05-07**: 15 dispatch_jobs ga4_mp filhos criados via `/tmp/pgquery/replay-ga4-purchase-skips-v2.mjs` (15/15 succeeded — cascata determinística resolveu client_id pra todos). 15 Purchase Guru históricos recuperam atribuição GA4. Aguarda agregação GA4 (~24h delay) pra aparecer nos reports.

- **DISPATCH-REPLAY-ENDPOINT-STANDALONE-MODE — RESOLVIDO 2026-05-07** (commits `4dd703d` + `4eff5f9`). Endpoint `POST /v1/dispatch-jobs/:id/replay` rodava em standalone mode desde Sprint 8 — ninguém nunca exercitou o replay em prod. Fix: `buildDispatchReplayRoute(db)` factory em [`apps/edge/src/index.ts:476`](apps/edge/src/index.ts#L476) com 3 closures (getDispatchJob, createReplayJob, insertAuditEntry). Pegadinha encontrada: schema `audit_log` usa `before/after/requestContext` (não `metadata/requestId`) — mapeamento consistente com integrations-sendflow. Bug adicional `c.get('workspace_id')` vazio causando erro Postgres 22P02 (UUID cast) — fix L4eff5f9 aceita header `X-Workspace-Id` como fallback (placeholder até auth Sprint 6).
...
- **T-14-009-FOLLOWUP (Sprint 14, descoberto 2026-05-06)**: refator de `apps/edge/src/dispatchers/google-ads-conversion/client.ts` para aceitar `accessToken` direto (alternativa ao `oauth: OAuthConfig` que faz refresh interno). Estado atual: `buildGoogleAdsConversionDispatchFn` em index.ts usa `resolveGoogleAdsCredentials` (helper T-14-003) e passa o tuple OAuth completo para o client — que faz 2º refresh interno. Implicação: (a) gasto duplicado de refresh por dispatch (latency +200ms), (b) `invalid_grant` cai em `server_error` (transient retry) em vez de `oauth_token_revoked` (skip + DB mark `oauth_token_state='expired'`). `buildEnhancedConversionDispatchFn` já usa `getGoogleAdsAccessToken` direto (sem essa pendência). Refator estimado: ~30min — adicionar parâmetro opcional `accessToken?` ao `sendConversionUpload`; quando presente, pular refresh interno; manter compat com chamadores legados. Owner: dispatcher-author.
...
Estado anterior: SPRINT 16 Onda 2 entregue (2026-05-07).
               ...
               ✅ Commits 4bde77f + 4dd703d + 4eff5f9, deploys edge sucessivos.
               ✅ ADR-032 registrado, OQ-012 FECHADA.
               ✅ Cascata 4 níveis em apps/edge/src/dispatchers/ga4-mp/
                  client-id-resolver.ts (resolveClientIdExtended):
                  self → sibling (filtro temporal) → cross_lead (phone→email)
                  → deterministic (uuidv5(workspace_id, lead_id))
               ✅ Endpoint /v1/dispatch-jobs/:id/replay agora persiste de verdade
                  (8 sprints rodando em standalone test mode).
               ✅ Backfill 15/15 Purchase Guru históricos succeeded — recuperam
                  atribuição GA4 (delay agregação ~24h).
               ✅ 263/263 dispatcher tests verde + 15 unit tests novos do resolver.

               ====================================================================

Estado anterior: SPRINT 16 Onda 1 entregue (2026-05-07).
