...
export const leads = pgTable(
  'leads',
  {
    ...
    // BR-PRODUCT-001: hierarquia monotônica de relacionamento — promoção nunca regride.
    // Valores: 'contato'(0) < 'lead'(1) < 'cliente'(2) < 'aluno'(3) < 'mentorado'(4).
    // Atualizada por lib/lifecycle-promoter.ts a partir de compras de produtos categorizados
    // (products.category) e progressão no funil. chk_leads_lifecycle_status enforces valid values.
    lifecycleStatus: text('lifecycle_status').notNull().default('contato'),
  ...
  (t) => ({
    // BR-PRODUCT-001: lookup acelerado para listagens filtradas por categoria
    // de relacionamento dentro do workspace; partial em status='active' evita
    // poluir índice com leads merged/erased.
    workspaceLifecycleIdx: index('idx_leads_workspace_lifecycle')
      .on(t.workspaceId, t.lifecycleStatus)
      .where(sql`${t.status} = 'active'`),

    // chk_leads_lifecycle_status — valores canônicos da hierarquia monotônica.
    lifecycleStatusCheck: check(
      'chk_leads_lifecycle_status',
      sql`${t.lifecycleStatus} IN ('contato', 'lead', 'cliente', 'aluno', 'mentorado')`,
    ),
  }),
);
